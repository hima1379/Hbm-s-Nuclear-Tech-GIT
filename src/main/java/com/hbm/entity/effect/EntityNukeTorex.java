...
/**
 * NUCLEAR MUSHROOM CLOUD VISUALIZATION
 *
 * Toroidal Convection Simulation Explosion Effect
 * Simulates the characteristic mushroom cloud using ~20,000 cloudlet particles
 *
 * PHYSICS IMPLEMENTATION (Current):
 * - Toroidal (donut-shaped) convection flow using Euler rotation
 * - Particle-based cloudlet simulation for visual accuracy
 * - Condensation ring simulation (shock wave visualization)
 * - Buoyancy-driven updraft column (stem)
 * - Ground-level dust particle expansion
 *
 * PHYSICS TO BE IMPROVED (Future Enhancement):
 * Based on Taylor-Sedov blast wave theory and atmospheric physics:
 *
 * 1. FIREBALL EVOLUTION:
 *    - Initial X-ray heating phase (microseconds): T ~ 10^7 K
 *    - Isothermal expansion phase: R(t) = C × (E×t²/ρ)^(1/5)
 *    - Blackbody radiation phase: T ~ 3,000-7,700°C
 *    - Cooling phase with atmospheric mixing
 *
 * 2. BUOYANCY-DRIVEN RISE:
 *    - Archimedes force: F = (ρ_air - ρ_hot) × g × V
 *    - Terminal velocity: v = sqrt(2×g×H × (ΔT/T_ambient))
 *    - Mushroom cap formation at atmospheric inversion layer
 *
 * 3. ATMOSPHERIC DENSITY GRADIENT:
 *    - Barometric formula: ρ(h) = ρ₀ × exp(-h/H)
 *    - Scale height H = 8.5 km
 *    - Affects rise rate and final stabilization height
 *
 * 4. TEMPERATURE-DEPENDENT COLOR:
 *    - High temp (>6000 K): White/blue-white
 *    - Medium temp (3000-6000 K): Yellow/orange
 *    - Low temp (<3000 K): Red/brown (dust dominates)
 *    - Stefan-Boltzmann radiation cooling: dT/dt = -σ×T⁴/(ρ×Cp×R)
 *
 * 5. UNDERGROUND/CONFINED EXPLOSIONS:
 *    - Replace mushroom cloud with hemispherical fireball
 *    - Channeling effects in tunnels (directed particle flow)
 *    - Reflection off bedrock surfaces
 *    - Reduced atmospheric mixing
 *
 * ENERGY COUPLING (Implemented in ExplosionNukeRayBatched):
 * - Only 10-55% of thermal energy creates visual fireball
 * - Rest absorbed by atmosphere (invisible heating)
 * - Visual effects should scale with effective thermal energy
 *
 * References:
 * - Glasstone & Dolan (1977) Chapter II: Descriptions of Nuclear Explosions
 * - Taylor-Sedov-von Neumann blast wave theory
 * - Atmospheric physics: U.S. Standard Atmosphere (1976)
 *
 * @author HBM Nuclear Tech TeamFi
 */
