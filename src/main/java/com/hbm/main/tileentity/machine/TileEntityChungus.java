	...
	public void update() {
		...
		if(!world.isRemote) {
			...
			if(world.getTotalWorldTime() % 20 == 0) {
				System.out.println("[Chungus] ===== LEVIATHAN TURBINE STATUS =====");
				System.out.println("[Chungus] Position: " + pos);
				FluidStack inputFluid = tanks[0].getFluid();
				FluidStack outputFluid = tanks[1].getFluid();
				System.out.println("[Chungus] Input Tank: " +
					(inputFluid != null ? inputFluid.getFluid().getName() + " " + inputFluid.amount : "EMPTY") +
					" / " + tanks[0].getCapacity() + " mB");
				System.out.println("[Chungus] Output Tank: " +
					(outputFluid != null ? outputFluid.getFluid().getName() + " " + outputFluid.amount : "EMPTY") +
					" / " + tanks[1].getCapacity() + " mB");
				System.out.println("[Chungus] Expected input type: " + types[0].getName());
			}
			...
			if(outs == null) {
				if(world.getTotalWorldTime() % 20 == 0) {
					System.out.println("[Chungus] ERROR: No recipe found for " + types[0].getName());
				}
				return;
			}
			...
			int cycles = Math.min(processMax, processSteam);  // Removed processWater constraint for continuous operation

			// Debug logging every second
			if(world.getTotalWorldTime() % 20 == 0) {
				System.out.println("[Chungus] Recipe: " + (Integer)outs[2] + " mB input -> " +
					(Integer)outs[1] + " mB output + " + (Integer)outs[3] + " HE");
				System.out.println("[Chungus] Calculated cycles: processMax=" + processMax +
					", processSteam=" + processSteam + " (output constraint removed)");
				System.out.println("[Chungus] Actual cycles: " + cycles);
				System.out.println("[Chungus] Power production this tick: " + ((Integer)outs[3] * cycles) + " HE/tick");
				System.out.println("[Chungus] Total stored power: " + power + " / " + maxPower + " HE");
			}

			tanks[0].drain((Integer)outs[2] * cycles, true);
			int outputAmount = (Integer)outs[1] * cycles;
			int filled = tanks[1].fill(new FluidStack(types[1], outputAmount), true);

			// Debug: warn if output is being discarded due to full tank
			if(world.getTotalWorldTime() % 20 == 0 && filled < outputAmount) {
				int discarded = outputAmount - filled;
				System.out.println("[Chungus] WARNING: Output tank full (" + tanks[1].getFluidAmount() + "/" +
					tanks[1].getCapacity() + " mB), discarded " + discarded + " mB spent steam (power generation continues)");
			}
