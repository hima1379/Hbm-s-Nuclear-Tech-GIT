		...
		public List<String> getFancyStats() {
			...
			case CONTROL:
				if(this.data.hasKey("color")) {
					...
					if(col >= 0 && col < TileEntityRBMKControlManual.RBMKColor.values().length)
						stats.add(TextFormatting.YELLOW + I18nUtil.resolveKey("rbmk.control." + TileEntityRBMKControlManual.RBMKColor.values()[col].name().toLowerCase()));
				}
			case CONTROL_AUTO:
