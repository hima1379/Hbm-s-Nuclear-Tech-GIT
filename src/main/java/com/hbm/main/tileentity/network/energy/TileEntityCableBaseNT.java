...
/**
 * Base class for energy cables with PowerNet support.
 *
 * This cable system supports both long and BigInteger energy values:
 * - long範囲内（~922京）では高速なlong演算を使用
 * - long最大値を超えた場合は自動的にBigIntegerに切り替わる
 * - longを使用している機械には自動的にlong範囲にクランプして変換
 *
 * EnergyValue support:
 * - transferPower(long): Legacy method for backward compatibility
 * - transferPowerEV(EnergyValue): New method supporting values beyond long range
 */
