...
public class ItemBoltgun extends Item implements IAnimatedItem {
    ...
    public boolean isBolt(ItemStack stack) {
        ...
        for(ItemStack bolt : bolts) {
            if(bolt.isItemEqual(stack)) return true;
        }
        return false;
    }
    ...
    public boolean onLeftClickEntity(ItemStack stack, EntityPlayer player, Entity entity) {
        ...
        boolean playingCreative = player.capabilities.isCreativeMode;
        for(int i = 0; i < player.inventory.getSizeInventory(); i++) {
            ItemStack invStack = player.inventory.getStackInSlot(i);
            if(playingCreative || isBolt(invStack)){
                if (!world.isRemote) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (b instanceof IToolable && ((IToolable) b).onScrew(world, player, pos.getX(), pos.getY(), pos.getZ(), facing, hitX, hitY, hitZ, hand, ToolType.BOLT)) {
            ...
            if ((materials.isEmpty() || InventoryUtil.doesPlayerHaveAStacks(player, materials, true)) && result.getValue().block != null) {
                IBlockState resultBlock = result.getValue().block.getStateFromMeta(result.getValue().meta);
